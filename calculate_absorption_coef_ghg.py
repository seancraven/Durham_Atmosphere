...
def main():
    ...
    for gas, ids in Ghg.ids.items():
        for alt in altitudes:
            wav_number, od = ghg_od_calculate(gas, alt)
            for nu, tau in zip(wav_number, od):
                try:
                    conn.execute("INSERT INTO optical_depths VALUES(?,?,?,?)",
                        (ids, alt,nu,tau)
                        )
                except sqlite3.IntegrityError:
                    sql = """SELECT * from optical_depths
                        ...
                        """
                    cur = conn.cursor()
                    cur.execute(sql,(Ghg.ids[gas],alt, nu))
